      ...
      .prompt([
        {
          ...
          type: "confirm",
          message:
            "Would you like to save this choice so that I use it by default when passing --development-team without arguments next time?"
        }
      ])
      .then(answers => {
        if (answers.dosave) {
      ...
      .prompt([
        {
          name: "searchortype",
          message:
            "Would you like me to search for a valid development team for you?",
          type: "confirm"
        }
      ])
      .then(answers => {
          ...
          inquirer
            .prompt([
              {
                name: "basedir",
                message:
                  "Where should I search for your existing XCode projects?",
                default: homedir,
                validate: answer => {
            ...
            .then(answers => {
              ...
              const command =
                "find " +
                answers.basedir +
                ' | grep -m500 "project\\.pbxproj" | xargs -L1 -JABC grep "evelopmentTeam" "ABC" 2>/dev/null | sort | uniq';
              ...
              cpp.exec(command, { encoding: "utf8" }).then(out => {
                ...
                const trimmed = lines
                  ...
                  .filter(line => {
                    return line.startsWith("DevelopmentTeam");
                  })
                  .map(line => {
                    var x = line.trim();
                ...
                if (trimmed.length) {
                  ...
                  console.log(
                    "I found no  development teams, let's try typing."
                  );
                  resolve(getFromTyping());
                }
              });
      ...
      .prompt([
        {
          ...
          validate: answer => {
            if (!answer) {
              ...
              console.log(
                "A valid development team ID is 10 digits long. this was not. Try again?"
              );
              return false;
            } else {
              return true;
